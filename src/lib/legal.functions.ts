...
export const analyzeDocument = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<Result<Analysis>> => (await import("./legal.server")).runAnalyze(data));

export const askDocument = createServerFn({ method: "POST" })
  .inputValidator((d: unknown) => AskInput.parse(d))
  .handler(async ({ data }): Promise<Result<string>> => (await import("./legal.server")).runAsk(data));

export const compareDocuments = createServerFn({ method: "POST" })
  .inputValidator((d: unknown) => PairSchema.parse(d))
  .handler(async ({ data }): Promise<Result<Comparison>> => (await import("./legal.server")).runCompare(data));

export const askComparison = createServerFn({ method: "POST" })
  .inputValidator((d: unknown) => AskComparisonInput.parse(d))
  .handler(async ({ data }): Promise<Result<string>> => (await import("./legal.server")).runAskComparison(data));
